...
テナントは「この AI エージェントや開発者に読ませてよい知識の集合」を分ける境界です。
...
リポジトリごとのテナントを標準にしない理由は、全体知識と個別リポジトリ知識を
横断して検索しにくくなり、リポジトリ名変更や分割統合がアクセス制御の変更に
なってしまうためです。
...
を見てください。
機密テナントや顧客リポジトリでは、GitHub Actions に mem0 の service token を
置くこと自体がリスクになります。
その場合は Actions 直結ではなく、ローカル clone からの同期、Tailscale 経由の同期、
private network 内の self-hosted runner、または write-only ingestion gateway を使います。
...
この service token は mem0 API へ到達できる鍵です。
漏えい時の影響を抑えるため、対象 repo を限定し、public repository や fork pull request に
渡さないでください。
高機密データでは GitHub Actions 直結を避けます。
`MEM0_API_KEY` は、mem0 API ランタイム側にも同じ値を設定した場合に
Bearer token として検証されます。
...
ただし、mem0 連携ではユーザーの検索語を先に匿名化する使い方を標準にはしません。
主な連携先として考えているのは、mem0 に書き込む前に本文を匿名化する経路です。

現時点の問題は、mem0 に生の本文を保存している場合、検索前にプロンプトを匿名化すると
mem0 検索の精度が落ちることです。
たとえば MCP 検索前に顧客名、リポジトリ名、API 名、ファイル名が置換されると、
匿名化後の検索語が mem0 内の生の本文と一致しにくくなります。

TODO として、任意の取り込み時匿名化、つまり `sanitize-on-ingest` を追加します。
匿名化が必要なテナントでは、GitHub Actions や Python 取り込み CLI が
チャンク本文を `agent-privacy-guard` に通してから mem0 に書き込みます。
