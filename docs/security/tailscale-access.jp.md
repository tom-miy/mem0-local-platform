...
ただし、GitHub Actions に mem0 のサービストークンを置きたくない高機密リポジトリでは、
Tailscale 経由のローカル同期やプライベートネットワーク内の self-hosted runner を使います。
...
- 高機密リポジトリの同期: Tailscale 経由のローカル同期、またはプライベートネットワーク内の self-hosted runner
- 外部の自動化エージェント: Cloudflare Tunnel + Cloudflare Access
- 自分の端末から自宅サーバへ接続: Tailscale
- サーバ上の Docker Compose 内部通信: Docker DNS

## Docker Compose の起動

Tailscale 経由で使う場合は、通常の Docker Compose 構成に Tailscale 用 override を重ねます。
...
HTTPS 証明書は Tailscale 側で終端され、Docker Compose 側の `mem0` と `mcp` は
...
通常は `tailscale cert` で証明書ファイルを発行して Docker Compose 側へ渡す必要はありません。
...
`127.0.0.1` に公開された Docker Compose サービスへ転送します。
...
mem0 のサービストークンを GitHub Actions に置きたくない場合は、GitHub-hosted runner からの
...
- 外部 LLM や外部埋め込みプロバイダを使う場合、検索結果や登録内容がプロバイダに送られる可能性があります。
