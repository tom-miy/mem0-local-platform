...
デフォルトの Docker Compose 実行環境には `ollama` サービスが含まれるため、
Docker Compose 内では `OLLAMA_BASE_URL=http://ollama:11434` を使えます。
Ollama をホスト側で動かす場合は
...
Docker Compose 起動後にデフォルトモデルを取得します。
...
開発中は便利ですが、Docker Compose だけでは完結しなくなります。

## Ollama Cloud

ホストされた Ollama 互換エンドポイントや API キーが必要な場合は、
`MEM0_CONFIG_FILE` で明示します。長い mem0 設定は `.env` に直接書かず、
YAML ファイルに分けます。
...
Docker Compose で使う場合は、`mem0` サービスへファイルをマウントします。
...
`compose.mem0-config.yml` は `./mem0.config.yml` を `mem0` サービスの
`/app/mem0.config.yml` にマウントします。`MEM0_CONFIG_FILE` は
`/app/mem0.config.yml` にします。

`MEM0_CONFIG_FILE` は `.json` ファイルも受け付けます。
`MEM0_CONFIG_JSON` も短い実験用としては引き続き使えます。
`MEM0_CONFIG_JSON` と `MEM0_CONFIG_FILE` は同時に指定しません。
