...
MCP 設定用の `install.sh` は設定例を表示するだけです。表示内容を確認してから、
...
読み取り可能テナントは `mem0.policy.yml` に書きます。
...
Docker Compose 内から mem0 API に接続する場合:
...
Docker Compose の `mcp` サービスを Cloudflare Tunnel で公開する場合は、MCP の
ホスト名を Cloudflare に設定します: `mem0-mcp.example.com -> http://mcp:8010`
