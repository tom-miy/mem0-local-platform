...
その secret を読めるワークフローは mem0 API へ到達できます。
現在の API では書き込み用と検索用のエンドポイントが同じ API ホストにあるため、
機密性の高いテナントや顧客リポジトリでは Actions 直結を標準にしないでください。
その場合は、ローカル clone からの同期、Tailscale 経由の同期、private network 内の
self-hosted runner、または write-only ingestion gateway を使います。
...
実務運用では設定してください。
GitHub Actions には同じ値を `MEM0_API_KEY` secret として渡します。
...
ただし、mem0 へ到達できる secret を広く配るほど漏えい時の影響が大きくなります。
...
機密テナントへ接続できる service token に `--visibility all` は使わないでください。
...
接続できる場合でも、GitHub Actions に mem0 の service token を置きたくない
